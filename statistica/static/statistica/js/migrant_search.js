...
function showInfoModal(data){
    ...
    data.forEach(item => {
        td += '<td>' + item['id'] + '</td>';
        td += '<td>' + item['user'] + '</td>';
        td += '<td>' + '<a href="http://127.0.0.1:8000/analytic/migrant-detail/'+ item['id'] + '" target="_blank">'  + item['case_name'] + '</a></td>';
...
function getRight() {
    const url = document.getElementById('migrant_data').textContent;
