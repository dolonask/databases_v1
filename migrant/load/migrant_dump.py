def load_migrants(apps, schema_editor):
    ...
    print('Source')
    Source = apps.get_model("migrant", "Source")
    source = Source(id=1, name="Интервью с пострадавшим", active=True)
    source.save()
    source = Source(id=2, name="Сообщение из СМИ", active=True)
    source.save()
    source = Source(id=3, name="Информация от работодателя", active=True)
    source.save()
    source = Source(id=4, name="Информация от юриста", active=True)
    source.save()
    source = Source(id=5, name="Информация от НКО, правозащитной организации", active=True)
    source.save()
    source = Source(id=6, name="Информация от правоохранительных органов", active=True)
    source.save()
    source = Source(id=7, name="Информация от государственных органов", active=True)
    source.save()
    source = Source(id=8, name="Информация от посольства/консульства", active=True)
    source.save()
